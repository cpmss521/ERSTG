...
class Token:
    def __init__(self, tid: int, index: int, span_start: int, span_end: int, phrase: str,vocab_id: int):
        ...
        self._span_end = span_end  # end of token span in document after Bert tokenization(exclusive)
        self._phrase = phrase
...
class TokenSpan:
    ...
    @property
    def span_token(self):
        return self._tokens[0].index,self._tokens[-1].index+1 # end of word in document without Bert tokenization(exclusive)
...
class Document:
    def __init__(self, doc_id: int, tokens: List[Token], entities: List[Entity], relations: List[Relation],
                 encoding: List[int], dep_label_indices: List[int], dep: List[int],
...
class Dataset(TorchDataset):
    ...
    def __init__(self, label, rel_types, entity_types, neg_entity_count,
                 neg_rel_count, max_span_size, over_rate):
        self._label = label
    ...
    def create_document(self, tokens, entity_mentions, relations, doc_encoding,
                        jdep_label_indices, jdep, jpos_indices) -> Document:
    ...
    def __getitem__(self, index: int):
        ...
        if self._mode == Dataset.TRAIN_MODE:
            return sampling.create_train_sample(doc, self._neg_entity_count, self._neg_rel_count,
                                                self._max_span_size, len(self._rel_types), self._over_rate)
        else:
